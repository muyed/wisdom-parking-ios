...
- (void)zwlimitCounter_swizzled_dealloc {
    ...
    @try {
        self.isHasobserver ? [self removeObserver:self forKeyPath:@"layer.borderWidth"] : nil;
...
- (BOOL)isHasobserver
{
    return [objc_getAssociatedObject(self, &isHasobserver) boolValue];
}
- (void)setIsHasobserver:(BOOL)isHasobserver
{
    objc_setAssociatedObject(self, &isHasobserver, @(isHasobserver), OBJC_ASSOCIATION_ASSIGN);
